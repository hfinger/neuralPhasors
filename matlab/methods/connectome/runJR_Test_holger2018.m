...
c = 135;
C = [c, c*0.8, c*0.25, c*0.25, 0]; % connectivity strength (can sometimes be interpreted as average synaptic contacts)
...
for k=1:length(drivStrengths)
  for l=1:length(drivFreqs)
    ...
    for i=1:numIters
      ...
      [ dx, da ] = JansenRitUpdate2018( x, a, Pp, Pe, Ke, Ki, Ka, Cpe, Cpi, Cep, Cip, Cii, de1, de2, di1, di2, e0, u0, r, S0 );
      x = x + dx * dt;
      a = a + da * dt;
