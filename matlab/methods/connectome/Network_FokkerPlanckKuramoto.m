function ys = Network_FokkerPlanckKuramoto(C,D,k,f,v,t_max,dt,sampling,sig_n,d,verbose,approx,invertSin,startState)
...
N          = size(C,1);
C          = k*C*dt;            % Scale the coupling strengths per time step
if invertSin
  C = -C;
end
...
if isempty(startState)
  y(:,1:n_td) = 2*pi*rand(N,n_td);
else
  y(:,1:n_td) = startState(:,end-n_td+1:end);
end
...
if v>0
    for c = 1:n_cycles
        ...
        for t = n_td:n_tpc-1
            dy = 2*pi*f*dt + sig_noise*randn(N,1);
            for n = 1:N
                for p = id{n}
                    dy(n) = dy(n) + C(n,p)*sin(y(p,t-stepsDelay(n,p)) - y(n,t));
                end
            end
            y(:,t+1) = y(:,t)+dy;
        end
