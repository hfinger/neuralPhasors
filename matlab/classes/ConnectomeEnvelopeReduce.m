...
%       this.params.ConnectomeEnvelopeReduce.results.Dim1.Avg = false;
    ...
    function run(this)
      ...
      if isfield(p,'eegSubjIds') && ~isfield(p,'eeg') && ~isfield(p,'sim')
        this.params.ConnectomeEnvelopeReduce.eeg.subj.Ids = p.eegSubjIds;
        this.params.ConnectomeEnvelopeReduce.eeg.subj.Avg = true;
        this.params.ConnectomeEnvelopeReduce.eeg.day.Ids = [];
        this.params.ConnectomeEnvelopeReduce.eeg.day.Avg = true;
        this.params.ConnectomeEnvelopeReduce.eeg.prepost.Ids = 1;
    ...
    function compareSimExp = compareWithEEG(this,sim)
      ...
      if p.eegDatabase==4
        ...
        data.eeg.dimName = {'roi','roi','freq','subj','day','cond'};
        ...
        data.eeg.dimName = {'roi','roi','freq','subj','day','prepost','task'};
...
%         
      ...
      eegSubjDim = find(strcmp(compareSimExp.dimName,'subj'));
                ...
                for iterDim1=1:dimSize(iterateOverDims(1))
                  ...
                  for iterDim2=1:dimSize(iterateOverDims(2))
                    ...
                    for transp=1:2
                      ...
                      RhoMatch{transp} = repmat(diag(metricVal),[1 size(metricVal,2)]);
                      RhoMatch{transp} = RhoMatch{transp}(nonDiagIds);
                      RhoNomat{transp} = metricVal(nonDiagIds);
                      [~,pval]=ttest(RhoMatch{transp}(:),RhoNomat{transp}(:));
                      if transp==1
                        compareSimExp.permTest.(metrics0{m0}).(metrics1{m1}).(metrics2{m2}).fixEeg(iterDim1,iterDim2) = pval;
                      elseif transp==2
                        compareSimExp.permTest.(metrics0{m0}).(metrics1{m1}).(metrics2{m2}).fixSim(iterDim1,iterDim2) = pval;
                      end

                      %% extract nonmatching matrix:
                      ur = triu(metricVal,1);
                      bl = tril(metricVal,-1);
                      if transp==1
                      ...
                      allNonmatch = metricVal;
                      allNonmatch(logical(eye(size(allNonmatch)))) = 0;
                      nonmatchingMeans = sum(allNonmatch,2) / (size(allNonmatch,2)-1);
                      matching = diag(metricVal);
                      [~,pval]=ttest(matching,nonmatchingMeans);
                      if transp==1
                        compareSimExp.permTest.(metrics0{m0}).(metrics1{m1}).(metrics2{m2}).fixEegNonmatchedAvg(iterDim1,iterDim2) = pval;
                      elseif transp==2
                        compareSimExp.permTest.(metrics0{m0}).(metrics1{m1}).(metrics2{m2}).fixSimNonmatchedAvg(iterDim1,iterDim2) = pval;
                      end
                      
                      %% do paired t-test where the nonmatching values are before averaged for each column:
                      nonmatchingMedian = median(nonmatch,2);
                      matching = diag(metricVal);
                      [~,pval]=ttest(matching,nonmatchingMedian);
                      if transp==1
                        compareSimExp.permTest.(metrics0{m0}).(metrics1{m1}).(metrics2{m2}).fixEegNonmatchedMedian(iterDim1,iterDim2) = pval;
                      elseif transp==2
                        compareSimExp.permTest.(metrics0{m0}).(metrics1{m1}).(metrics2{m2}).fixSimNonmatchedMedian(iterDim1,iterDim2) = pval;
                      end
...
%           for m2=1:length(metrics2)
%             
%             siz = size(compareSimExp.(metrics0{m0}).(metrics1{m1}).(metrics2{m2}));
%             
%             if length(siz) > 2
%               continue;
%             elseif siz(2) == 1
%               figure(1);
%               plot(compareSimExp.(metrics0{m0}).(metrics1{m1}).(metrics2{m2}));
%               title([metrics0{m0} '.' metrics1{m1} '.' metrics2{m2}])
%               ylabel(metrics2{m2})
%               xlabel(dimNames{1})
%               set(gca,'XTick',1:dimSizes(1));
%               set(gca,'XTickLabel',dimLabels{1})
%               set(gcf,'PaperUnits','inches','PaperPosition',[0 0 8 6])
%               print('-dpng','-r72',fullfile(plotDir,[metrics0{m0} '_' metrics1{m1} '_' metrics2{m2} '.png']))
%             else
%               figure(1);
%               imagesc(compareSimExp.(metrics0{m0}).(metrics1{m1}).(metrics2{m2}));
%               title([metrics0{m0} '.' metrics1{m1} '.' metrics2{m2}])
%               colormap hot;
%               % set(gca,'clim',[-1 1])
%               colorbar
...
%       end
      ...
      if length(compareSimExp.dimName)==2 && sum(strcmp(compareSimExp.dimName,'subj')) && sum(strcmp(compareSimExp.dimName,'subjId'))
        ConnectomeEnvelopeReduce.plotStructure(compareSimExp.overFreq, compareSimExp, 'result.overFreq', 'result_overFreq', plotDir, {'pval'})
        ConnectomeEnvelopeReduce.plotStructure(compareSimExp.perFreqAvg, compareSimExp, 'result.perFreqAvg', 'result_perFreqAvg', plotDir, {'pval'})
      end
      ...
      if isfield(compareSimExp,'permTest')
        ConnectomeEnvelopeReduce.plotStructure(compareSimExp.permTest.overFreq, compareSimExp.permTest, 'ttest.overFreq', 'ttest_overFreq', plotDir, {'pval'});
        ConnectomeEnvelopeReduce.plotStructure(compareSimExp.permTest.perFreqAvg, compareSimExp.permTest, 'ttest.perFreqAvg', 'ttest_perFreqAvg', plotDir, {'pval'});
      end
      ...
      if isfield(compareSimExp,'samples') && isfield(compareSimExp.samples,'cohy_eegBest') && isfield(compareSimExp.samples,'cohy_simBest')
        ...
        if length(compareSimExp.dims_eeg)==4
          [I,J] = ind2sub(compareSimExp.dims_eeg(3:end),compareSimExp.samples.bestEegId);
        elseif length(compareSimExp.dims_sim)==3
          I = compareSimExp.samples.bestEegId;
          J = compareSimExp.samples.bestSimId;
        else
          [I,J] = ind2sub(compareSimExp.dims_sim(3:end),compareSimExp.samples.bestSimId);
        end
        descParam = [dimNames{1} '=' num2str(dimLabels{1}{I}) ' and ' dimNames{2} '=' num2str(dimLabels{2}{J})];
  ...
  methods(Static)
    ...
    function plotStructure(structure, inSpec, caption, filename, plotDir, excludeFields)
      ...
      if isstruct(structure)
        ...
        for f=1:length(fnames)
          if sum(strcmp(excludeFields,fnames{f}))==0
            ConnectomeEnvelopeReduce.plotStructure(structure.(fnames{f}), inSpec, [caption '.' fnames{f}], [filename '_' fnames{f}], plotDir, excludeFields);
          end
        end
        ...
        elseif siz(2) == 1
          ...
          plot(structure);
          title(caption)
          xlabel(inSpec.dimName{1})
          ylabel(caption)
          set(gca,'XTick',1:size(structure,1));
          set(gca,'XTickLabel',inSpec.dimLabels{1})
          set(gcf,'PaperUnits','inches','PaperPosition',[0 0 8 6])
