classdef ConnectomeEnvelope < Gridjob
  ...
  methods
    ...
    function this = ConnectomeEnvelope(varargin)
      ...
      this.params.ConnectomeEnvelope.savePowerSpectrum = false;
      this.params.ConnectomeEnvelope.saveFilteredPowerSpectrum = false;
      ...
      this.params.ConnectomeEnvelope.overwrite = false;
    ...
    function run(this)
      ...
      if ~exist(fullfile(savepath,['FCsimJob' num2str(this.currJobid) '.mat']),'file') || this.params.ConnectomeEnvelope.overwrite
      
        input = safeload(fullfile(this.workpath, this.params.ConnectomeEnvelope.inFileRates));
        ...
        if p.applyLeadField
          ...
          if strcmp(p.applyLeadFieldMethod,'perROI') || strcmp(p.applyLeadFieldMethod,'perVertex')
            
            lf = load(fullfile(paths.databases,'SC_Bastian','surfaces','projectionMat','caAll_projectionMat.mat'));
            if input.param.subjId<1
              ...
              lf.projMatRoi = lf.projMatRoi{input.param.subjId};
              lf.projMatRoiPerVertex = lf.projMatRoiPerVertex{input.param.subjId};
            end
            ...
            if input.param.subjId<1
              tmp = cellfun(@(x) cell2mat(permute(x,[1 3 2])), lf.lf_all([1:4 6:10]), 'UniformOutput', false);
              tmp = mean(cell2mat(permute(tmp,[1 3 4 2])),4);
              lf = permute(num2cell(tmp,[1 2]),[3 1 2])';
            else
          ...
          elseif strcmp(p.applyLeadFieldMethod,'perROI')
            
            rate = lf.projMatRoi*rate;
            
          elseif strcmp(p.applyLeadFieldMethod,'perVertex')
            
            rate = lf.projMatRoiPerVertex*rate;
        ...
        if p.savePowerSpectrum
          
          Pxx = cell(size(rate,1),1);
          window = 8192;
          noverlap = 1024;
          for k=1:size(rate,1)
            [Pxx{k},PxxFreqz] = pwelch(rate(k,:),window,noverlap,[],this.Fs,'onesided');
          end
          save([savepath '/powerSpectrum_job' num2str(this.currJobid)],'Pxx','PxxFreqz');
        ...
        for b=1:length(p.sigBandpass)
          ...
          if p.saveFilteredPowerSpectrum
            
            Pxx = cell(size(sourceBP,1),1);
            for k=1:size(sourceBP,1)
              window = 2048;
              noverlap = 1024;
              [Pxx{k},PxxFreqz] = pwelch(sourceBP(k,:),window,noverlap,[],this.Fs,'onesided');
            end
            save([savepath '/powerSpectrum_band' num2str(b) '_job' num2str(this.currJobid)],'Pxx','PxxFreqz');
