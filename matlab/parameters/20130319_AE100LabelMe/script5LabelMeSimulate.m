...
params.ApplyWeights.actFcn = @(x) feval(@(x2) bsxfun(@rdivide,x2,0.01+sum(x2,3)), 1./(1+exp(-x)) ); % sigmoid and then normalize to sum=1
paramsAll{1} = params;
...
params.FeatureCovariance.inNumFeatures = 400;
