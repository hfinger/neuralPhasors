...
params.Gridjob.jobname = 'SigmVarMuLBFGS.m';
...
params.Autoencoder.outWeightsFolder = 'SigmVarMuLBFGS.m';
...
params.Autoencoder.mu = {1, 10};        % weight of modular topographic sparsity penalty term
params.Autoencoder.muPvalue = 0.5;        % Psi_mu = mu * ( muFeatureKernel * (muConvKernel * y2.^2) + muEpsilon ) ^ muPvalue
params.Autoencoder.muConvKernel = fspecial('gaussian', 7, 2);
params.Autoencoder.muFeatureKernel = {[],ones(3,3)}; % if empty then no combination in feature dimension, otherwise it should be a matrix of size (hiddenSize,hiddenSize)
...
params.Autoencoder.saveintervalpngKeep = 20;
