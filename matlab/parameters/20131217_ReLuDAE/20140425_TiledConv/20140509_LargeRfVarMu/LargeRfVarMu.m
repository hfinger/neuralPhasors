...
params.Gridjob.jobname = 'ReLuDAELargeRfVarMu';
...
params.Autoencoder.outWeightsFolder = 'ReLuDAELargeRfVarMu';
...
params.Autoencoder.hiddenSize = 6;
...
params.Autoencoder.mu = 5;        % weight of modular topographic sparsity penalty term
params.Autoencoder.muPvalue = 0.5;        % Psi_mu = mu * ( muFeatureKernel * (muConvKernel * y2.^2) + muEpsilon ) ^ muPvalue
params.Autoencoder.muConvKernel = {ones(3,3), fspecial('gaussian', 7, 2)};
params.Autoencoder.muFeatureKernel = {[],ones(6,6),diag(ones(1,5),1)+diag(ones(1,5),-1)+diag(ones(1,6),0)+diag(ones(1,1),5)+diag(ones(1,1),-5)}; % if empty then no combination in feature dimension, otherwise it should be a matrix of size (hiddenSize,hiddenSize)
...
params.Autoencoder.validationIntervalLog = 10;
...
params.Autoencoder.batchsize = 20; %[] means full batch
