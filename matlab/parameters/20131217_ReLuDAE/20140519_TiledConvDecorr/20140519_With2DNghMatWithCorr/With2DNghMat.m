...
nuCovWeightMat = linearDecoderNghMat( [2 2 2 2],[2 2 5 5],[false false true true]);
nuCovWeightMat(nuCovWeightMat==1) = -1;
nuCovWeightMat(nuCovWeightMat==0) = 1;
params.Autoencoder.nuCovWeightMat = nuCovWeightMat;
...
params.Autoencoder.plotPermute = @(W) reshape(permute(reshape(W,[size(W,1) size(W,2) size(W,3) 2 2 5 5]),[1 2 3 4 6 5 7]),[size(W,1) size(W,2) size(W,3) 10 10]);
