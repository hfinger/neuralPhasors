...
params.JansenRitConnectomePaper.xCollExtendedIdxs = 1:4;
...
% set driver params
idx = randperm(length(C),3);

params.JansenRitConnectomePaper.drivPos = num2cell(idx); % indices of network nodes to be driven
params.JansenRitConnectomePaper.drivScale = 0; %1e-3; %num2cell([1,2,4,8,16]); % driver strength [mV]
params.JansenRitConnectomePaper.drivPO = 0; % phase offset of drivers
params.JansenRitConnectomePaper.drivFreq = 10; %num2cell([8:0.5:14]); % frequency of driver [Hz]
