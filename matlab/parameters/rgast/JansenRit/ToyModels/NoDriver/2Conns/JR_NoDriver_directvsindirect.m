...
params.Gridjob.jobname = 'JR_NoDriver_directvsindirect';
...
d_ratio = 0.25; % relative position where internode will be placed
d_direct = [0:200]; % distance between outer nodes for direct conn

for d=1:length(d_direct)
    D = zeros(3);
    D(1,2) = d_ratio*d_indirect;
    D(2,3) = (1-d_ratio)*d_indirect;
    D(1,3) = d_direct(d);
    D = D + D';
    Delays{d} = D;
end
