...
for d=1:length(d213)
    ...
    D(1,2) = d213(d) * fixRatio;
    D(2,1) = d213(d) * fixRatio;
    
    D(1,3) = d213(d) * (1-fixRatio);
    D(3,1) = d213(d) * (1-fixRatio);
...
params.JansenRitConnectomePaper.drivScale = num2cell(ones(1,5)*0.1); % 0.25, 0.5 driver strength [mV]
