...
params.Gridjob.runOnlyJobIds = [7040, 8846];
...
driv_positions = cell(1,size(pairs,1));
for i=1:size(pairs,1)
    driv_positions{i} = pairs(i,:);
end

%% set driver params
params.JansenRitConnectomePaper.drivPos = driv_positions; % indices of network nodes to be driven
params.JansenRitConnectomePaper.drivScale = 0.5; % 0.25, 0.5 driver strength [mV]
...
%job = JansenRitConnectomePaper('/net/store/nbp/projects/phasesim/workdir/Holger/2018_JR/ConnectomeTwoDrivers/AllNodePairs11Hz500uVsameSeed/temp_Connectome/jobDesc.mat');
%job.finishJobs()