...
for k=1:8
    ...
    all_tMax(k) = results{k}.simResult.sim.tMax - 30;
end
...
%% combine different sims:
coh_12_180 = mean(coh_vals(:,1:2), 2);
coh_123_360 = mean(coh_vals(:,1:3), 2);

%coh_vals = cat(2, coh_vals, coh_12_180, coh_123_360);
%all_tMax = cat(2, all_tMax, 180, 360);
...
figure(1);
imagesc(all_corrs)
set(gca, 'XTick', 1:length(all_tMax))
set(gca, 'XTickLabel', all_tMax)
set(gca, 'YTick', 1:length(all_tMax))
set(gca, 'YTickLabel', all_tMax)
colorbar;
xlabel('Tmax')
ylabel('Tmax')
title('compare Tmax correlation')
saveas(gcf, fullfile( path_results, 'compare_Tmax_corr.png'))
...
xlabel('Tmax')
ylabel('Tmax')
title('compare Tmax absolute distance')
saveas(gcf, fullfile( path_results, 'compare_Tmax_dist.png'))
