...
params.JansenRitConnectomePaper.k = 10; %num2cell([3, 10]); %30; %num2cell(round(22:2:34)); %global connection strength scaling
...
params.JansenRitConnectomePaper.sampling = num2cell([2.^(1:6) 2.^(1:6)]); % sampling every x steps
params.JansenRitConnectomePaper.sampleNoiseEvery = num2cell(cellfun(@(x) x/2, params.JansenRitConnectomePaper.sampling)); % sampling every x steps
params.JansenRitConnectomePaper.dt = num2cell(cellfun(@(x) 0.001/x, params.JansenRitConnectomePaper.sampling)); % simulation step size [seconds]
params.JansenRitConnectomePaper.use_rk4 = num2cell([false(1,6) true(1,6)]);
...
params.JansenRitConnectomePaper.initSampRem = 125; %initial interval to remove [seconds]
